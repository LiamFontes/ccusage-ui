...
function BlocksPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900">Billing Blocks</h2>
        <p className="text-gray-600 mt-1">5-hour billing block analysis and projections</p>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {isLoading ? (
              ...
              nonGapBlocks.map((block) => (
                <tr
                  key={block.id}
                  className={`hover:bg-gray-50 ${block.isActive ? 'bg-green-50' : ''}`}
                >
                    ...
                    <div className="flex flex-wrap gap-1">
                      {block.models
                        ...
                        .map((model) => (
                          <span
                            key={model}
                            className="inline-flex px-2 py-0.5 text-xs rounded bg-gray-100"
                          >
    ...
    <div className="bg-green-50 border border-green-200 rounded-lg p-6">
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h3 className="text-lg font-medium text-green-800">Active Block</h3>
          <p className="text-green-700 mt-1">
            Started {formatDateTime(block.startTime)} with {block.entries} entries
          </p>

          {burnRate && (
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <p className="text-sm text-green-600">Burn Rate</p>
                <p className="text-xl font-semibold text-green-900">
                  {formatCompactNumber(burnRate.tokensPerMinute)}/min
                </p>
              </div>
              <div>
                <p className="text-sm text-green-600">Cost Rate</p>
                <p className="text-xl font-semibold text-green-900">
          ...
          {projection && (
            <div className="mt-4 p-3 bg-green-100 rounded-lg">
              <p className="text-sm text-green-800">
                <span className="font-medium">Projection:</span> This block will use approximately{' '}
                {formatCompactNumber(projection.totalTokens)} tokens and cost{' '}
                {formatCurrency(projection.totalCost)} by completion.
              </p>
            </div>
          )}
        </div>
